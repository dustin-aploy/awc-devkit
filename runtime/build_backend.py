"""Minimal offline build backend for awc-runtime.
...
SRC = ROOT / "src"
DIST_NAME = "awc_runtime"
PROJECT_NAME = "awc-runtime"
...
ENTRY_POINTS = "[console_scripts]\nawc-runtime = awc_runtime.cli:main\n"
...
def _metadata_text() -> str:
    return "\n".join(
        [
            ...
            f"Name: {PROJECT_NAME}",
            f"Version: {VERSION}",
            "Summary: Lightweight reference runtime for AI Work Contract (AWC), formerly ARP.",
...
def _wheel_text() -> str:
    return "\n".join(
        [
            "Wheel-Version: 1.0",
            "Generator: awc-runtime-build-backend",
...
def _build_metadata_dir(metadata_directory: str) -> str:
    ...
    (metadata_dir / "top_level.txt").write_text("awc_runtime\n", encoding="utf-8")
...
def _write_wheel(wheel_directory: str, editable: bool) -> str:
    ...
    entries.extend(
        [
            ...
            (f"{DIST_INFO}/top_level.txt", b"awc_runtime\n"),
        ]
    )
